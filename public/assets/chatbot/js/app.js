        ...
        .then(response => {
            if (response.ok) {
                ...
                setTimeout(() => {
                    ...
                    chatbox.messages.pop();
                    chatbox.updateChatText();
                    let nameSentMessage = { name: "Marita", message: "Submit Successfully. We're committed to guiding you every step of the way. Do you have a referral letter to Dr. O’Dwyer? <button class=\"submit-name-button\" style=\"background-color: #130185; color: #fff; padding: 8px 16px; border: none; border-radius: 5px; cursor: pointer; margin-top: 10px;\" onclick=\"chooseResponse('yes')\">Yes</button> <button class=\"submit-name-button\" style=\"background-color: #130185; color: #fff; padding: 8px 16px; border: none; border-radius: 5px; cursor: pointer; margin-top: 10px;\" onclick=\"chooseResponse('No')\">No</button>" };
                    chatbox.messages.push(nameSentMessage);
                    chatbox.updateChatText();
                }, loadingTimeout);
            ...
            if (!this.promptDisplayed) {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        this.messages.push({
                            name: "Marita",
                            message: "Hi, I am your free virtual health assistant provided by Dr. O’Dwyer from the Beacon Hospital. Please take a moment to read our <a href='/privacy' target='_blank'>consent and privacy form</a>\n" +
                                "If you agree to the consent and privacy policy and to receive health updates via SMS <br>' <button class=\"submit-name-button\" style=\"background-color: #130185; color: #fff; padding: 8px 16px; border: none; border-radius: 5px; cursor: pointer; margin-top: 10px;\" onclick=\"chooseResponse('agree')\">Agree</button>"
                        });
